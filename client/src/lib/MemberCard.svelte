...
<button
    class="relative flex py-1 my-8 gap-8 w-fit items-center"
    title={"Copy " + member.name + "'s email address"}
    data-value={member.email}
    on:click={copyEmail}
>
    <div
        id={nameToId(member.name) + "-alert"}
        class="notification text-white absolute hidden z-10 top-1 right-1 gap-2 bg-emerald-600 px-4 py-1 rounded"
    >
